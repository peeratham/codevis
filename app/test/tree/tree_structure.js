function Tree(rootName){
  ...
  that.addNode = function(nodePath){
    _addNode(_root,nodePath);
  }

  that.removeNode = function(nodePath){
    _removeNode(_root,nodePath);
  }

  

  function _calculatePath(root){
    var nodes = [], i = 0;
...
function _addNode(root, nodePath) {
...
function _removeNode(root, path){
  var path = formatPath(path);
      path = path.substring(1,path.length); //drop front slash
  var parts = path.split('/'); 

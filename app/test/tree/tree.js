...
// d3.json("structure.json", function(data) {   //comment/uncomment this line to use real data
  ...
  diagram = TreeDiagram(div);   //give it a dom element so it knows where to draw in the page
  flatten(data);    //generate the path data for each node
  diagram.data(data); //assign the data
  diagram.update(); //call update to draw it to the screen

  //example of adding/ modifying tree
  setTimeout(function(){  //adding node
    addNode(data, '/root/test');  //addNode(root, pathOfNodeFromRoot)
    diagram.update();
  }, 1000); //execute all of this after waiting 1000 millisecs
...
// }); //comment/uncomment this line to use real data

/**
Anything (function, variable) you want to make available to the outside use 'that' object
for example, we a variable _diameter inside TreeDiagram function
if you want to be able to adjust it outside just attach a function to 'that' object like:

//approach 1: you can only read the value
that.diameter = function(){   
    return _diameter;
}
//approach 2: you can update and read value 
that.diameter = function (d) {
      if (!arguments.length) return _diameter;    //when no parameter is provided, we can get info about number of arguments 
      _diameter = d;                              //passed to the function by calling arguments.length (a javascript feature)
      return that;            //so if no args, when user just call diagram.diameter() it will just return _diameter
  };                          //however, if user call diagram.diameter(500) then it will assign 500 to internal variable _diameter
...
function TreeDiagram(p) {
  ...
  function _update(source){
     //compute new tree layout
      ...
      //find paretn's position to be used to exit the children when toggle
      var src_element = gnode.filter(function(d){
        return d.path==source.path
      });  
      ...
      //enter links 
       ...
       link.exit().transition()
         .duration(duration)
         .attr("d", function(d) {
          var parent_el = gnode.filter(function(d0){ if(d0 ){ return d0.name == d.source.name;}});
          console.log(parent_el);
          console.log(d);
          if(parent_el[0].length){
           parent_transform =  parent_el.attr('transform');
         }
